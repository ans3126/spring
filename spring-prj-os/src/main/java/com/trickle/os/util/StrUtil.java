...
public class StrUtil {
	...
	public static String selectQuery(String tableName, String[] columns, String where) {
		return "SELECT " + String.join(",", columns) + " FROM " + tableName + " " + where;
	}
	...
	public static String getPagingQuery(String tableName, String orderByColumn, String[] columns) {
		...
		return String.format(
				"SELECT %s FROM (" + "SELECT %s FROM (" + "SELECT %s, ROWNUM AS RN FROM (" + "SELECT %s FROM %s "
						+ "ORDER BY %s DESC ) )" + "WHERE RN >= ?) WHERE ROWNUM <= ?",
	...
	public static String toPrettyJson(Object obj) {
		try {
			...
			;
			String jsonString;
			jsonString = objectWriter.writeValueAsString(objectMapper.valueToTree(obj));
			return jsonString;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static String toJson(Object obj) {
		try {
			...
			if (obj instanceof String) {
				jsonString = objectWriter.writeValueAsString(objectMapper.readTree((String) obj));
			} else {
				jsonString = objectWriter.writeValueAsString(obj);
			}
			return jsonString;
		} catch (Exception e) {
			e.printStackTrace();
